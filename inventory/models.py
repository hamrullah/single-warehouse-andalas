...
class Product(TimeStampedModel):
    ...
    is_active = models.BooleanField(default=True)
    qty_on_hand = models.DecimalField(max_digits=12, decimal_places=2, default=0)  # single-warehouse
    class Meta:
        ordering = ["name"]
...
class Transaction(TimeStampedModel):
    ...
    def clean(self):
        if self.quantity <= 0:
            raise ValidationError("Quantity harus > 0")
        # prevent negatif saat OUT (ketika instance baru atau quantity bertambah)
        ...
        if self.pk:  # update
            ...
            current = current - old_delta  # rollback dulu
        if current + delta < 0:
